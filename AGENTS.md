...
This document is the project's source of truth for context, vocabulary, and architectural decisions. It is the first file Codex (or any contributor) should read when working in this repo.
...
We store conversation metadata alongside the file state. Current Codex and Codex hooks write normalized message rows into the SQLite index, store tool payloads as content-addressed blobs, and keep raw agent transcript snapshots as blobs when the host exposes a transcript path. Older data may still use the chained `Transcript` blob: `{ prev: hash, new_messages: [hash...] }`.
...
| **Session** | A continuous stream of agent activity from one host (one Codex process, one SDK run). Has its own ref (branch). |
| **Agent** | An identity within a session. Sub-agents (e.g. spawned via Task tool) get their own agent_id but share session lineage. |
| **Hook** | The integration point with the agent host. Codex and Codex adapters normalize host payloads into shared capture events. |
...
- **Codex and Codex hook documentation** — the host integration points we depend on. Host payloads should stay confined to adapter code.

---

## Status

**Active development.** Core storage, hook capture, log, blame, show, sessions, and init are implemented. Codex and Codex are the reference host integrations.
...
7. **Multi-tool unification.** Codex and Codex now share the same capture engine. Future adapters should preserve the same `origin`, `session_id`, and `turn_id` invariants.
